...
const NuevoPassword = () => {
  ...
  useEffect(() => {
    const comprobarToken = async () => {
      try {
        await axios(`http://localhost:4000/api/veterinarios/olvide-password/${token}`)
        setTokenValido(true)
      } catch (error) {
        setAlerta({
  ...
  return (
    <>
      <h1 className="text-sky-600 font-black text-6xl">Reestablece tu password y no pierdas acceso a tus <span className="text-slate-700">pacientes</span></h1>
